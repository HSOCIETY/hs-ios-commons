...
extension String {
    ...
    func getValidCoupon() -> String? {
        do {
            let regex = try NSRegularExpression(pattern: "[a-z|A-Z]{2}\\d{3}[|a-z|A-Z]{4}"/* AB690FWVM */, options: [])
    ...
    /*
     ...
     ex) 11000 -> 11,000
     */
    func numberFormattedString() -> String {
        return String.numberFormattedString(intValue: Int(self)!)
    }

    static func numberFormattedString(intValue value: Int) -> String {
        if value < 100 {
            return String(value)
        }
